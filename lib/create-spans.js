/****
 * Takes an array of words and builds up a new array of strings that pass the
 * plugin parameters
 * @param {target} element
 * @param {words} array
 * @param {font_size} integer
 * @param {min_chars_per_line} integer
 * @param {font_ratio} float
 * @return {spans} array
****/
//var verifyString = require('./verify-string');

var createSpans = function(target, words, font_size, min_chars_per_line, font_ratio) {
  console.log(this);
  ...
  var current_string = '';
  var working_string = '';
  //var chars_per_line = Math.max(min_chars_per_line, Math.floor(parent_width / (font_size * font_ratio))) || 20;
  ...
  for (var w = 0; w < words.length; w++) {
    ...
    var a = verifyString(current_string);
    var b = verifyString(working_string);
